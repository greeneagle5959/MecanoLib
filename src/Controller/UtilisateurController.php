...
// la methode pour inscrire un utilisateur (client)
...

    {
        ...
        if ($user->getIs2fa()) {
            ...
            $googleAuthenticatorClass = 'Sonata\\GoogleAuthenticator\\GoogleAuthenticator';
            if (!class_exists($googleAuthenticatorClass)) {
                return $this->json(['erreur' => 'Le module Google Authenticator n\'est pas installé'], 500);
            }

            $g = new $googleAuthenticatorClass();
            $timestep = 1;
    ...
    private function tvaFr(string $siren): ?string
{
    ...
    // prendre les 9 premiers chiffres
    ...
    // formatage + retour
    return 'FR' . str_pad($cle, 2, '0', STR_PAD_LEFT) . $siren;
}
}
